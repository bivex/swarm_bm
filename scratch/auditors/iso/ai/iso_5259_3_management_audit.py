...
def calculate_score(check: ISO5259Part3Check, match_count: int) -> Tuple[int, str, str]:
    if match_count == 0:
...
def scan_project(path: str, project_name: str) -> List[ISO5259Part3Check]:
    checks = [
        ISO5259Part3Check(
            clause_id="4.1",
            clause_ref="Data Quality Requirements",
            normative_text="Organization SHALL define data quality requirements for AI/ML use including dimensions, thresholds and methods.",
            category="DQ_REQUIREMENTS",
            weight=10,
            search_terms=["data quality requirements", "threshold values", "measurement methods", "documented requirements", "traceable to objectives"]
        ),
        ISO5259Part3Check(
            clause_id="5.1",
            clause_ref="Data Quality Management System",
            normative_text="Establish data quality policy, roles, planning, control, assurance and improvement.",
            category="DQ_MANAGEMENT_SYSTEM",
            weight=10,
            search_terms=["data quality policy", "roles and responsibilities", "quality planning", "quality control", "quality assurance", "quality improvement"]
        )
    ]
...
def print_report(checks: List[ISO5259Part3Check], project_name: str):
    safe_name = "".join([c if c.isalnum() else "_" for c in project_name]).lower()
    app_data = Path.home() / ".gemini" / "antigravity-cli" / "brain" / "b1a8b172-4960-462a-bad1-43d8b7e774ad"
    app_data.mkdir(parents=True, exist_ok=True)
    report_file = app_data / f"iso_5259_3_{safe_name}.md"
    
    with open(report_file, "w") as f:
        f.write(f"# ISO/IEC 5259-3 Management Framework Audit: {project_name}\n\n")
...
def main():
    if len(sys.argv) < 3:
        print("Usage: python iso_5259_3_management_audit.py <path> <project_name>")
        sys.exit(1)
