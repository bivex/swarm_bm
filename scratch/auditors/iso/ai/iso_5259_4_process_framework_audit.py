...
def calculate_score(check: ISO5259Part4Check, match_count: int) -> Tuple[int, str, str]:
    if match_count == 0:
...
def scan_project(path: str, project_name: str) -> List[ISO5259Part4Check]:
    checks = [
        ISO5259Part4Check(
            clause_id="4.1",
            clause_ref="Data Quality Processes",
            normative_text="Process for data quality assessment, improvement, monitoring, and provenance tracking.",
            category="DQ_PROCESSES",
            weight=10,
            search_terms=["profiling", "completeness check", "statistical analysis", "imputation", "deduplication", "normalization", "continuous quality tracking", "provenance tracking", "lineage"]
        ),
        ISO5259Part4Check(
            clause_id="5.1",
            clause_ref="Data Governance Integration",
            normative_text="Integration with data governance: catalogue, lineage, access control, and compliance.",
            category="DQ_GOVERNANCE",
            weight=10,
            search_terms=["data catalogue", "metadata", "data dictionaries", "data access control", "GDPR", "compliance with data regulations"]
        )
    ]
...
def print_report(checks: List[ISO5259Part4Check], project_name: str):
    safe_name = "".join([c if c.isalnum() else "_" for c in project_name]).lower()
    app_data = Path.home() / ".gemini" / "antigravity-cli" / "brain" / "b1a8b172-4960-462a-bad1-43d8b7e774ad"
    app_data.mkdir(parents=True, exist_ok=True)
    report_file = app_data / f"iso_5259_4_{safe_name}.md"
    
    with open(report_file, "w") as f:
        f.write(f"# ISO/IEC 5259-4 Process Framework Audit: {project_name}\n\n")
...
def main():
    if len(sys.argv) < 3:
        print("Usage: python iso_5259_4_process_framework_audit.py <path> <project_name>")
        sys.exit(1)
