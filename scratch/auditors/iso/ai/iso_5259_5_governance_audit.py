...
def calculate_score(check: ISO5259Part5Check, match_count: int) -> Tuple[int, str, str]:
    if match_count == 0:
...
def scan_project(path: str, project_name: str) -> List[ISO5259Part5Check]:
    checks = [
        ISO5259Part5Check(
            clause_id="4.1",
            clause_ref="AI Data Governance",
            normative_text="Data governance framework for AI/ML, stewardship roles, policy, and lifecycle management.",
            category="DATA_GOVERNANCE",
            weight=10,
            search_terms=["data governance framework", "data owner", "data steward", "data custodian", "data quality policy"]
        ),
        ISO5259Part5Check(
            clause_id="4.4",
            clause_ref="Data Lifecycle Management",
            normative_text="Data lifecycle management for AI (acquisition to disposal).",
            category="DATA_LIFECYCLE",
            weight=10,
            search_terms=["data lifecycle", "acquisition", "preparation", "archiving", "disposal"]
        ),
        ISO5259Part5Check(
            clause_id="5.1",
            clause_ref="Organizational Data Quality Management",
            normative_text="Executive sponsorship, metrics reporting, continuous improvement, and training.",
            category="DATA_GOVERNANCE",
            weight=10,
            search_terms=["executive sponsorship", "metrics and reporting", "continuous improvement", "training and competence"]
        )
    ]
...
def print_report(checks: List[ISO5259Part5Check], project_name: str):
    safe_name = "".join([c if c.isalnum() else "_" for c in project_name]).lower()
    app_data = Path.home() / ".gemini" / "antigravity-cli" / "brain" / "b1a8b172-4960-462a-bad1-43d8b7e774ad"
    app_data.mkdir(parents=True, exist_ok=True)
    report_file = app_data / f"iso_5259_5_{safe_name}.md"
    
    with open(report_file, "w") as f:
        f.write(f"# ISO/IEC 5259-5 Governance Framework Audit: {project_name}\n\n")
...
def main():
    if len(sys.argv) < 3:
        print("Usage: python iso_5259_5_governance_audit.py <path> <project_name>")
        sys.exit(1)
